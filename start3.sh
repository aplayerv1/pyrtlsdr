...
# Configuration variables for 151 MHz center frequency
ffreq=150.8e6
lfreq=151.2e6
sfreq=151e6
duration=1200
...
for file in *.fits; do
    if [[ -f "$file" ]]; then
        ...
        # Append _151MHz to the filename
        filename_w_151="${filename_w}_${fileappend}"

        # Rename the file with _151MHz appended
        mv "$file" "raw/${filename_w_151}.fits"
        
        # Confirm the file is in raw/ directory
        ls raw/

        # Create directory to store processed images
        mkdir -p "images/${filename_w_151}"

        # Preprocess the file using preprocess.py
        echo "Starting Preprocess"
        python3 preprocess.py -i "raw/${filename_w_151}.fits" -o raw/ --center_frequency $sfreq
        ...
        echo "Starting Sound"
        pf="${filename_w_151}.fits.txt"  # Assuming correct pattern for processed file

        python3 tosound.py "raw/$pf" "sound/${filename_w_151}.wav" --samplerate 48000

        echo "Starting Processing"
        python3 process5.py -f "raw/$pf" -i "raw/${filename_w_151}.fits" -o "images/${filename_w_151}/" --start_time 0 --end_time $duration --tolerance $tol --chunk_size $chunk --fs $srf --center-frequency $sfreq

        # Generate Heatmap
        echo "Starting Heatmap"
        python3 heatmap.py -i "raw/${filename_w_151}.fits" -o "images/${filename_w_151}/" --fs $srf --num-workers $workers --nperseg 2048
        ...
        find /home/server/rtl/pyrtl/raw/ -type f -mmin +720 -exec rm -r {} \;
        find /home/server/rtl/pyrtl/images/ -type f -mmin +720 -exec rm -r {} \;
        find /home/server/rtl/pyrtl/sound/ -type f -mmin +720 -exec rm -r {} \;
    fi
done
